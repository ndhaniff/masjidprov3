...
function Relatives() {
  const { relatives, qariah } = usePage().props
  ...
  const selectAction = (action, id) => {
    switch (action) {
      case 'delete':
        ...
        break
      default:
        Inertia.visit('/qariah/relatives/show/' + id, { method: 'get', data: { panelMode: 'edit' } })
    }
  }
  ...
  return (
    ...
    <div>
      <div className="relative">
        ...
        <p>Tanggungan bagi anak qariah bernama <span className="font-semibold">{qariah.name}</span></p>
        <div className='absolute flex items-center top-5 right-5'>
          <Button basic color="blue" onClick={() => Inertia.visit('/qariah', { 'method': 'get' })}>
          ...
          <Button primary onClick={() => Inertia.visit('/qariah/relatives/add/' + qariah.id, { 'method': 'get' })}>
            <div className="flex items-center">
      ...
      <div className="rounded-md shadow bg-white p-5">
        {relatives.data.length ? <Table compact selectable>
          <Table.Header>
            <Table.Row>
          ...
          <Table.Body>
            {relatives.data.length && relatives.data.map((relative, index) => (
              <Table.Row key={'relative-' + index}>
                <Table.Cell>{relative.full_name}</Table.Cell>
                <Table.Cell>{relative.new_ic}</Table.Cell>
                <Table.Cell>{relative.age}</Table.Cell>
                <Table.Cell>{_i('relative.sex', relative.sex)}</Table.Cell>
                <Table.Cell>
                  <Button.Group basic compact>
                    <Button onClick={e => { e.preventDefault(); selectAction('view', relative.id) }}>
                    ...
                    <Button onClick={e => { e.preventDefault(); selectAction('delete', relative.id) }}>
            ...
            <Table.Row className='bg-gray-50'>
              <Table.Cell colSpan='7'>
                <div className="flex items-center justify-between">
                  <div>Jumlah Keseluruhan: <b>{relatives.total} Orang</b></div>
                  {relatives.data.length > 10 && <Menu floated='right' pagination>
                    <Menu.Item icon>
                      <Link href={relatives.prev_page_url}><Icon name='chevron left' /></Link>
                    </Menu.Item>
                    {relatives.links.map((link, index) => index !== 0 && index !== (relatives.links.length - 1) &&
                    ...
                    <Menu.Item icon>
                      <Link href={relatives.next_page_url}>
